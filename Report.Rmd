...
## Variational Autoencoders for single-cell transcriptomics data 
...
#### 2.1.2 The objective loss function
...
<p style="text-align: center;">$\mathcal{L}_{AE} = \frac{1}{N} \sum_{i=1}^{N}||\mathbf{x}^{(i)} - \hat{\mathbf{x}}^{(i)}||^{2}$</p>
...
![Illustration of an autoencoder network.[6]](Autoencoder-768x590.png)
<p style="text-align: center;">A schematic of an autoencoder network. Figure retrieved from [6]</p>
...
![Illustration of VAE network.[7]](VAE1.png)
<p style="text-align: center;">A simplified schematic of a VAE network. Figure retrieved from [7]</p> 

#### 2.2.2 The objective loss function
...
<p style="text-align: center;">$p_{\theta}(\mathbf{x}) = \int p_{\theta}(\mathbf{x}|\mathbf{z})p_{\theta}(\mathbf{z})d\mathbf{z}$</p> 
...
This bound is known as ELBO (evidence lower bound) and it is used to for the network optimization. The first term is the reconstruction error and the KL-divergence term works as a regularization term. By maximizing ELBO the optimization indirectly minimizes KL-divergence from $p(\mathbf{z}|\mathbf{x})$ to $q(\mathbf{z})$. 

#### 2.2.3 Reparametrization trick
There is one more obstacle for VAEs to learn representations of the data. To optimize the parameters of the distributions the backpropagation algorithm needs to propagate through random variables (latent samples $z \sim q(\mathbf{z}|\mathbf{x})$) meaning it requires a way to compute the gradient of the expectations of random variables. Gradient can be computed only for deterministic variables so a reparametrization trick needs to be done to to enable the backpropagation propagate through the random nodes. Now introducing a new parameter $\epsilon$ as auxilliary random noise as input to the network allows us to reparametrize $z$ in a way that it is no more the source of randomness.

![](reparam.png)
<p style="text-align: center;">A schematic of the reparametrization trick. Figure retrieved from [8]</p> 

#### 2.2.4 $\beta$-constraint
...
![](VSC.png)
<p style="text-align: center;">Schematic representation of the variational sparse coding model (right) compared to a standard VAE (left). Figure retrieved from [10]</p> 

#### 2.3.2 $\beta$-constraint
Similarly to $\beta$-VAE, we introduce a $\beta$-constraint to the VSC-ELBO as well.

## 3. Experiments and results

In this section we present the code, results, training times and architectures of our neural network models. All of the models were trained for 200 epochs. The code for the experiments can be found at [https://github.com/okunator/Variational-Autoencoders](https://github.com/okunator/Variational-Autoencoders) in a notebook-file 'run_experiments.ipynb'. For the models with $\beta$-constraint, a value of 5.0 was used for the performance validations. Also tests with different values were tested without validations to see the effect on the tsne-plots. Adam optimizer was used for each training of the models. Learning rate was set 0.001 for the optimizer for each model.
...
The autoencoder we built has a deep architecture. Suppose that the input gene expression vector for one cell is denoted as $\mathbf{x} \in \mathcal{R}^{d}$, latent space as $\mathbf{z} \in \mathcal{R}^{6}$ (meaning that the number of latent features is 6) and the reconstructed output vector denoted as $\hat{\mathbf{x}} \in \mathcal{R}^d$. These vectors are put through of a network that is composed of linear layers with ReLU activation functions in the following order:

- Encoder network
    - $ \hspace{4cm} \mathbf{h}_1 = W_1\mathbf{x}+b_1, \hspace{3.08cm} \mathbf{h}_1 \in \mathcal{R}^{128}$  
    - $ \hspace{4cm} \mathbf{h}_2 = ReLU(W_2\mathbf{h}_1+b_2), \hspace{1.3cm} \mathbf{h}_2 \in \mathcal{R}^{32}$
    - $ \hspace{4cm} \mathbf{h}_3 = ReLU(W_3\mathbf{h}_2+b_3), \hspace{1.3cm} \mathbf{h}_3 \in \mathcal{R}^{16}$ 
    - $ \hspace{4cm} \mathbf{z} = ReLU(W_4\mathbf{h}_3+b_4), \hspace{1.85cm} \mathbf{z} \in \mathcal{R}^{6}$ 
  
- Decoder network
    - $ \hspace{4cm} \mathbf{h}_5 = ReLU(W_5\mathbf{h}_4+b_5), \hspace{1.3cm} \mathbf{h}_5 \in \mathcal{R}^{16}$
    - $ \hspace{4cm} \mathbf{h}_6 = ReLU(W_6\mathbf{h}_5+b_6), \hspace{1.3cm} \mathbf{h}_6 \in \mathcal{R}^{32}$
    - $ \hspace{4cm} \mathbf{h}_7 = ReLU(W_7\mathbf{h}_6+b_7), \hspace{1.3cm} \mathbf{h}_7 \in \mathcal{R}^{128}$
    - $ \hspace{4cm} \hat{\mathbf{x}} = W_8\mathbf{h}_7+b_8, \hspace{3.33cm} \hat{\mathbf{x}} \in \mathcal{R}^{d}$

### 3.2 Variational Autoencoder with beta entanglement
